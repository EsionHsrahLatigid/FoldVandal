...
{
    (void) newAccentColor;
    foldvandalProcessor = dynamic_cast<FoldVandalPlugin*> (&processor);
    ...
    if (foldvandalProcessor != nullptr)
    {
        ...
        {
            foldvandalProcessor->setAuditionEnabled (! foldvandalProcessor->isAuditionEnabled());
        ...
        {
            foldvandalProcessor->setAuditionType (1 - foldvandalProcessor->getAuditionType());
            syncAuditionControls();
        };
...
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (foldvandalProcessor != nullptr && inputMeter != nullptr && outputMeter != nullptr)
    {
        const auto latestInputPeak = foldvandalProcessor->getInputPeakLevel();
        const auto latestPeak = foldvandalProcessor->getOutputPeakLevel();
...
{
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (foldvandalProcessor == nullptr || auditionButton == nullptr || auditionTypeButton == nullptr)
        return;

    auditionButton->setButtonText (foldvandalProcessor->isAuditionEnabled() ? "Audition On" : "Audition Off");
    auditionTypeButton->setButtonText (foldvandalProcessor->getAuditionType() == 0 ? "Saw" : "Pulse");
    static_cast<ehl::ui::CommandButton*> (auditionButton.get())->setSelected (foldvandalProcessor->isAuditionEnabled());
    static_cast<ehl::ui::CommandButton*> (auditionTypeButton.get())->setSelected (foldvandalProcessor->getAuditionType() != 0);
#endif
}
