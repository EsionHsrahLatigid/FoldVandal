...
namespace foldvandal::plugin
{
namespace
{
constexpr std::array<char, 4> stateMagic {{ 'F', 'V', 'D', '1' }};
...
constexpr std::array<std::array<float, presetParameterCount>, 4> presetValues {{
    {{ 0.48f, 0.00f, 0.62f, 0.38f, 0.56f, 1.00f, 0.82f }},
    {{ 0.74f, 0.31f, 0.86f, 0.22f, 0.72f, 0.94f, 0.70f }},
    {{ 0.36f, -0.18f, 0.44f, 0.74f, 0.34f, 1.00f, 0.90f }},
    {{ 0.92f, 0.46f, 0.95f, 0.52f, 0.88f, 0.86f, 0.62f }}
}};
...
FoldVandalPlugin::FoldVandalPlugin()
    : yup::AudioProcessor ("FoldVandal",
                           yup::AudioBusLayout ({
...
{
    parameters[drive] = makeParameter ("drive", "Drive", drive, 0.0f, 1.0f, presetValues[0][drive], yup::AudioParameter::ParameterUnit::Percent, 16.0f);
    parameters[bias] = makeParameter ("bias", "Bias", bias, -1.0f, 1.0f, presetValues[0][bias], yup::AudioParameter::ParameterUnit::Percent, 24.0f);
    parameters[fold] = makeParameter ("fold", "Fold", fold, 0.0f, 1.0f, presetValues[0][fold], yup::AudioParameter::ParameterUnit::Percent, 16.0f);
    parameters[symmetry] = makeParameter ("symmetry", "Symmetry", symmetry, 0.0f, 1.0f, presetValues[0][symmetry], yup::AudioParameter::ParameterUnit::Percent, 22.0f);
...
void FoldVandalPlugin::prepareToPlay (const yup::AudioSpec& spec)
{
...
void FoldVandalPlugin::releaseResources()
{
}

void FoldVandalPlugin::processBlock (yup::AudioProcessContext<float>& context)
{
...
bool FoldVandalPlugin::acceptsMidi() const noexcept
{
    return false;
}

bool FoldVandalPlugin::producesMidi() const noexcept
{
    return false;
}

int FoldVandalPlugin::getCurrentPreset() const noexcept
{
    return currentPreset.load (std::memory_order_relaxed);
}

void FoldVandalPlugin::setCurrentPreset (int index) noexcept
{
...
int FoldVandalPlugin::getNumPresets() const
{
    return static_cast<int> (presetNames.size());
}

yup::String FoldVandalPlugin::getPresetName (int index) const
{
...
void FoldVandalPlugin::setPresetName (int index, yup::StringRef newName)
{
    if (yup::isPositiveAndBelow (index, static_cast<int> (presetNames.size())))
        presetNames[static_cast<std::size_t> (index)] = newName;
}

yup::Result FoldVandalPlugin::loadStateFromMemory (const yup::MemoryBlock& data)
{
    int loadedPreset = 0;
...
yup::Result FoldVandalPlugin::saveStateIntoMemory (yup::MemoryBlock& data)
{
    return saveProductState (*this, data, stateMagic, stateVersion, currentPreset.load (std::memory_order_relaxed));
}

bool FoldVandalPlugin::hasEditor() const
{
#if FOLDVANDAL_HEADLESS_TEST
    return false;
#else
...
yup::AudioProcessorEditor* FoldVandalPlugin::createEditor()
{
#if FOLDVANDAL_HEADLESS_TEST
    return nullptr;
#else
...
float FoldVandalPlugin::getInputPeakLevel() const noexcept
{
    return static_cast<float> (inputPeakMilli.load (std::memory_order_relaxed)) * 0.001f;
}

float FoldVandalPlugin::getOutputPeakLevel() const noexcept
{
    return static_cast<float> (outputPeakMilli.load (std::memory_order_relaxed)) * 0.001f;
}

#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
void FoldVandalPlugin::setAuditionEnabled (bool shouldBeEnabled) noexcept
{
    auditionEnabled.store (shouldBeEnabled ? 1 : 0, std::memory_order_relaxed);
}

bool FoldVandalPlugin::isAuditionEnabled() const noexcept
{
    return auditionEnabled.load (std::memory_order_relaxed) != 0;
}

void FoldVandalPlugin::setAuditionType (int type) noexcept
{
    auditionType.store (std::clamp (type, 0, 1), std::memory_order_relaxed);
}

int FoldVandalPlugin::getAuditionType() const noexcept
{
    return auditionType.load (std::memory_order_relaxed);
}
#endif

void FoldVandalPlugin::advanceParameterHandles (int samplePosition) noexcept
{
    for (std::size_t i = 0; i < parameterHandles.size(); ++i)
    {
...
void FoldVandalPlugin::syncParameterValuesFromParameters() noexcept
{
    for (std::size_t i = 0; i < parameters.size(); ++i)
        currentParameterValues[i] = parameters[i]->getValue();
}

void FoldVandalPlugin::updateEngineParameters() noexcept
{
    foldvandal::FoldVandalParameters engineParameters;
    engineParameters.drive = currentParameterValues[drive];
    engineParameters.bias = currentParameterValues[bias];
    engineParameters.fold = currentParameterValues[fold];
    engineParameters.symmetry = currentParameterValues[symmetry];
...
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
StereoFrame FoldVandalPlugin::renderAuditionFrame() noexcept
{
...
{
    return new foldvandal::plugin::FoldVandalPlugin();
}
