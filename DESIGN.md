...
FoldVandal is a focused stereo asymmetric wavefolder. It is not an instrument in hosted formats: hosted VST3/AU processing must never generate sound from silence.

## Product Checklist

- [x] Product version is `0.1.0`.
- [x] App ID is `jp.ehl.foldvandal`.
- [x] Plugin ID is `jp.ehl.foldvandal`.
- [x] AU subtype is `FdVn`.
- [x] State header is product-unique: `FVD1`, version `1`.
- [x] Public host parameters are exactly `Drive`, `Bias`, `Fold`, `Symmetry`, `Tone`, `Mix`, and `Output`.
- [x] Plugin format is stereo input/output effect; `PLUGIN_IS_SYNTH` is off.
- [x] MIDI input/output is disabled.

## DSP Checklist

- [x] Signal path order is drive/bias -> 4x interpolated asymmetric fold -> smoothing -> tone/DC blocking -> dry/wet -> bounded output.
...
- [x] `Drive`, `Fold`, `Bias`, `Symmetry`, `Tone`, and gain/mix parameters have regression coverage.
...
- [x] Effect targets are `foldvandal_release_bundles`, `foldvandal_standalone_plugin`, `foldvandal_vst3_plugin`, and Apple-only `foldvandal_au_plugin`.
